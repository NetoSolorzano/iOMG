...
namespace iOMG
{
    public partial class pan_inicio : UserControl
    {
        string valant = "";
        string valnue = "";
        ...
        private void pan_inicio_Load(object sender, EventArgs e)
        {
            jaladat();
        ...
        private bool vali_alm(string codi)
        {
            bool retorna = false;
        ...
        {
            //const string DB_CONN_STR0 = "Server=localhost;Uid=root;Pwd=20430100344;Database=coopera;";
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i < advancedDataGridView1.Rows.Count; i++)
                            {
                                if (advancedDataGridView1.Rows[i].Cells["marca"].FormattedValue.ToString() == "True")
                                {
                                    ...
                                    string actua = "update almloc set reserva=@res,contrat=@con,marca=0 where id=@idr";
                                    MySqlCommand miact = new MySqlCommand(actua, cnx);
                                    miact.Parameters.AddWithValue("@res", advancedDataGridView1.Rows[i].Cells["reserva"].Value.ToString());
                                    miact.Parameters.AddWithValue("@con", advancedDataGridView1.Rows[i].Cells["contrat"].Value.ToString());
                                    ...
                                    while (dr.Read())
                                    {
                                        ...
                                        for (int i = 0; i < advancedDataGridView1.Rows.Count; i++)
                                        {
                                            if (advancedDataGridView1.Rows[i].Cells["id"].Value.ToString() == dr.GetString(7))
                                            {
                                                ...
                                                {
                                                    //advancedDataGridView1.Rows[i].Cells["chksalida"].Value = true;  // .Value = 1; 
        ...
        private void bt_expex_Click(object sender, EventArgs e)     // exporta a excel
        {
